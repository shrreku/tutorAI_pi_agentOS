...
export async function loadRehydrationTranscript(
  ...
  const orderedQuery = typeof (baseQuery as { orderBy?: unknown }).orderBy === "function"
    ? (baseQuery as { orderBy: (value: unknown) => { limit: (count: number) => Promise<Array<Record<string, unknown>>> } }).orderBy(
        desc(tutorTurns.turnIndex),
      )
...
function summarizeTools(value: unknown): string {
  ...
  const summary = tools
    .flatMap((entry) => {
      ...
      const toolName = typeof (entry as { toolName?: unknown }).toolName === "string"
      ...
      const status = typeof (entry as { status?: unknown }).status === "string"
        ...
        : "completed";
      const latencyMs = typeof (entry as { latencyMs?: unknown }).latencyMs === "number"
