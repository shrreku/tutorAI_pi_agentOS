...
export function buildLearningArtifactView(artifact: RawArtifact): LearningArtifactView {
  ...
  const claimRefs: NodeRef[] = (artifact.sourceClaimIds ?? []).map((refId) => ({
    refType: "claim",
    refId,
  }));
  const chunkRefs: NodeRef[] = (artifact.sourceChunkIds ?? []).map((refId) => ({
    refType: "chunk",
    refId,
  }));
  const objectiveRefs: NodeRef[] = idsFrom(payload.objectiveIds ?? payload.plannedObjectiveIds).map(
    (refId) => ({
      refType: "objective",
      refId,
    }),
  );
  const coverageRefs: NodeRef[] = idsFrom(payload.coverageItemIds ?? payload.coverageRefs).map(
    (refId) => ({
  ...
  const sections = sectionsForArtifact(
  ...
  return {
    ...
    lastUpdatedReason: stringOrNull(
      payload.lastUpdatedReason ?? payload.updatedReason ?? payload.approvalNote,
    ),
    sections,
...
export function buildWikiArtifactView(page: WikiLike): LearningArtifactView {
  const claimRefs: NodeRef[] = (page.sourceClaimIds ?? []).map((refId) => ({
    refType: "claim",
    refId,
  }));
  const chunkRefs: NodeRef[] = (page.sourceChunkIds ?? []).map((refId) => ({
    refType: "chunk",
    refId,
  }));
  ...
  return {
    ...
    studentAction:
      "Use this as the reference page while studying or asking the tutor for deeper explanation.",
    status: page.status,
    sourceRefs,
    claimRefs,
    coverageRefs: [],
...
function sectionsForArtifact(
  ...
  const summary = stringOrNull(
    payload.summary ?? payload.takeaway ?? payload.finalTakeaway ?? payload.description,
  );
  ...
  if (artifactType === "note") {
    pushMarkdown(
      ...
      markdown ?? summary,
      sourceRefs,
      "Regenerate this note with a short overview, key ideas, source-backed examples, common mistakes, and a review checklist.",
    );
    pushList(
      sections,
      "key_points",
      "Key points",
      payload.keyPoints ?? payload.points,
      "key_points",
      sourceRefs,
    );
    ...
    pushList(
      ...
      payload.formulas ?? payload.definitions,
      "formulae",
      sourceRefs,
    );
    ...
    pushMarkdown(
      ...
      "problem",
      "Problem",
      stringOrNull(payload.problemStatement ?? payload.problem) ?? title,
      sourceRefs,
      "Regenerate this worked example with a source-grounded problem statement.",
    );
    ...
    pushList(
      ...
      "steps",
      "Reasoning steps",
      payload.solutionSteps ?? payload.steps,
      "steps",
      sourceRefs,
    );
    pushMarkdown(
      ...
      "answer",
      "Final answer",
      stringOrNull(payload.finalAnswer ?? payload.finalTakeaway ?? payload.takeaway),
      sourceRefs,
      "Regenerate this example with a final answer and a reusable takeaway.",
    );
    ...
    pushMarkdown(
      ...
      "try_next",
      "Try next",
      stringOrNull(payload.tryNext ?? payload.nextPrompt),
      sourceRefs,
      "Add one nearby practice problem after regenerating.",
    );
    ...
    pushList(
      ...
      payload.symbols ?? payload.assumptions,
      "table",
      sourceRefs,
    );
    pushList(
      ...
      "examples",
      "Example usage",
      payload.exampleUsage ?? payload.examples,
      "steps",
      sourceRefs,
    );
    if (sections.length === 0)
      pushMarkdown(
        ...
        "Regenerate with columns for expression, meaning, assumptions, units, when to use it, and one example.",
        sourceRefs,
      );
    ...
    pushMarkdown(
      ...
      [payload.leftTitle, payload.rightTitle].filter(Boolean).join(" vs "),
      sourceRefs,
    );
    pushList(
      sections,
      ...
      payload.comparisonRows ?? payload.rows ?? payload.items,
      "comparison",
      sourceRefs,
    );
    pushMarkdown(
      ...
      "decision",
      "When to use each",
      [payload.whenToUseLeft, payload.whenToUseRight].filter(Boolean).join("\n\n"),
      sourceRefs,
    );
    pushMarkdown(
      sections,
      "confusion",
      "Common confusion",
      stringOrNull(payload.commonConfusion),
      sourceRefs,
    );
    if (sections.length === 0)
      pushMarkdown(
        ...
        "format",
        "Comparison format",
        "Regenerate with two concepts, comparison dimensions, when to use each, common confusion, and a checkpoint question.",
        sourceRefs,
      );
    ...
    pushMarkdown(
      ...
      "overview",
      "Practice goal",
      stringOrNull(payload.prompt ?? payload.summary),
      sourceRefs,
      "Regenerate with a focused practice goal and source-backed answer key.",
    );
    ...
    if (!Array.isArray(payload.questions) || payload.questions.length === 0)
      pushMarkdown(
        ...
        "question_format",
        "Question format",
        "Include recall, application, and misconception checks. Each question should have a reference answer and explanation.",
        sourceRefs,
      );
    ...
    pushMarkdown(
      ...
      stringOrNull(payload.prompt ?? payload.summary),
      sourceRefs,
      "Regenerate with cards that test one idea at a time.",
    );
    ...
    if (!Array.isArray(payload.cards) || payload.cards.length === 0)
      pushMarkdown(
        ...
        "card_format",
        "Card format",
        "Use concise fronts, precise backs, ambiguity warnings when needed, and concept links.",
        sourceRefs,
      );
    ...
    pushMarkdown(
      ...
      "goal",
      "Revision goal",
      stringOrNull(payload.goal ?? payload.summary),
      sourceRefs,
    );
    pushList(
      sections,
    ...
    pushMarkdown(
      sections,
      "goal",
      artifactType === "teaching_arc" ? "Lesson structure" : "Session goal",
      stringOrNull(payload.sessionGoal ?? payload.goal ?? payload.summary),
      sourceRefs,
    );
    pushList(
      sections,
      "sequence",
      "Teaching sequence",
      payload.sequence ?? payload.steps ?? payload.objectives,
      "steps",
      sourceRefs,
    );
    pushList(
      ...
      payload.checks ?? payload.diagnosticQuestions ?? payload.exitCriteria,
      "questions",
      sourceRefs,
    );
    ...
    pushList(
      ...
      "next",
      "Next recommendation",
      payload.nextActions ?? payload.actionItems,
      "steps",
      sourceRefs,
    );
    pushList(
      ...
      "artifacts",
      "Generated artifacts",
      payload.artifactRefs ?? payload.generatedArtifacts,
      "metadata",
      sourceRefs,
    );
    ...
    pushMarkdown(
      ...
      "definition",
      "Definition",
      stringOrNull(payload.definition ?? payload.summary),
      sourceRefs,
      "Regenerate this concept card with a precise source-backed definition.",
    );
    pushMarkdown(
      sections,
      "use",
      "When to use it",
      stringOrNull(payload.whenToUse ?? payload.useCase),
      sourceRefs,
      "Add when the student should apply this concept.",
    );
    pushList(sections, "examples", "Examples", payload.examples, "steps", sourceRefs);
    pushMarkdown(
      sections,
      ...
      stringOrNull(payload.commonConfusion),
      sourceRefs,
      "Add a common confusion and how to avoid it.",
    );
    ...
    pushMarkdown(
      ...
      "caption",
      "Diagram",
      stringOrNull(payload.caption ?? payload.summary ?? payload.description),
      sourceRefs,
    );
    pushList(
      sections,
      "elements",
      "Elements",
      payload.nodes ?? payload.elements,
      "metadata",
      sourceRefs,
    );
  }
  ...
  if (sections.length === 0) {
    pushMarkdown(
      ...
      "reference",
      title,
      markdown ?? summary ?? readableFallback(payload),
      sourceRefs,
      "No readable content is available yet.",
    );
  }
...
function studentActionForArtifact(type: string): string {
  ...
  return (
    map[type] ?? "Study the artifact, inspect its evidence, and ask the tutor for help when needed."
  );
}

function actionsForArtifact(
  type: string,
  status: string,
  sourceRefs: NodeRef[],
): LearningArtifactAction[] {
  const actions: LearningArtifactAction[] = [
    {
      id: type === "quiz" || type === "flashcards" ? "practice" : "study",
      label: type === "quiz" || type === "flashcards" ? "Practice" : "Study",
      intent: "primary",
    },
  ];
  actions.push({ id: "ask_tutor", label: "Ask tutor", intent: "secondary" });
  if (sourceRefs.length > 0)
    actions.push({ id: "open_source", label: "Open evidence", intent: "secondary" });
  if (status === "draft" || status === "proposed")
    actions.push({ id: "approve", label: "Approve", intent: "secondary" });
...
function normalizeNodeRefs(value: unknown): NodeRef[] {
  ...
  return value
    .map((item) =>
      isRecord(item) && typeof item.refType === "string" && typeof item.refId === "string"
...
function idsFrom(value: unknown): string[] {
  ...
  return value
    .map((item) =>
      typeof item === "string"
        ? item
        : isRecord(item) && typeof item.refId === "string"
          ? item.refId
          : null,
    )
...
function readableFallback(payload: Record<string, unknown>): string {
  const keys = Object.keys(payload)
    ...
    .slice(0, 8);
  return keys.length
    ? `Structured fields available: ${keys.join(", ")}.`
    : "No readable content is available yet.";
}
