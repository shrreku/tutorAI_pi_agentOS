  ...
  const [signals, currentEstimates, masteryRows, profileRows, sessionSummaries] = await Promise.all(
    [
      ...
      dbClient.db
        ...
        .where(
          and(
            eq(studentProfiles.notebookId, input.notebookId),
...
function summarizeMasteryEvidenceRow(row: {
  ...
  const correctnessLabel =
    typeof evidence.correctnessLabel === "string" ? evidence.correctnessLabel : "unknown";
  const confidence = typeof evidence.confidence === "number" ? evidence.confidence : null;
  const evidenceType =
    typeof evidence.evidenceType === "string" ? evidence.evidenceType : "unknown";
  const parts = [
  ...
  if (
    ...
    (correctnessLabel === "partial" || correctnessLabel === "incorrect")
  ) {
