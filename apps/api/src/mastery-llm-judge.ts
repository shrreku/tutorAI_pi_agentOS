...
export function createOpenRouterMasteryEvaluatorJudge(
  ...
  const judgeTimeoutMs = 6_000;

  return async (input: JudgeInput): Promise<MasteryEvaluatorJudgeResult> => {
    const controller = new AbortController();
    const timeoutHandle = setTimeout(() => controller.abort(), judgeTimeoutMs);
    ...
    try {
      ...
      if (error instanceof Error && error.name === "AbortError") {
        throw new Error(`Mastery evaluator timed out after ${judgeTimeoutMs}ms`);
      }
      throw error;
    } finally {
    ...
    if (!response.ok) {
      throw new Error(`Mastery evaluator failed (${response.status}): ${body.error?.message ?? "unknown_error"}`);
    }

    const text = body.choices?.[0]?.message?.content;
    if (!text) throw new Error("Mastery evaluator returned empty content.");
    const parsed = JSON.parse(text) as MasteryEvaluatorJudgeResult;
    return parsed;
  };
}
