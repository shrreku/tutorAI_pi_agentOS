...
import {
  ...
  learnerSafeValue,
  mapLearnerPrimaryActions,
  type PageReadiness,
} from "@studyagent/schemas";
import {
  resolveGenerationModeFromWikiRecord,
...
export function toLearnerFacingReferenceSurface(
  ...
  return learnerSafeValue({
    ...
    interactiveBlocks: (surface.interactiveBlocks ?? []).map((block) =>
      toLearnerFacingInteractiveBlock(block),
    ),
  });
}
...
async function withInteractiveBlocks(
  ...
  const evidenceRefs =
    input.evidenceRefs ?? surface.blocks.flatMap((block) => block.evidenceRefs ?? []);
...
export async function buildReferenceSurface(
  ...
  const base = (
    overrides: Partial<ReferenceSurface> &
      Pick<ReferenceSurface, "nodeRef" | "title" | "surfaceType">,
  ): ReferenceSurface => ({
    id: `surface_${nodeId}`,
    notebookId,
    summary: null,
    ...
    const livePlanBlock = buildLivePlanBlock(
      {
        ...
        completedObjectiveIds: (studyState.studyPlan?.completedObjectives ?? []).map(
  ...
  if (openTarget.kind === "concept" && openTarget.entity) {
    ...
    const claimRows = claimIds.length
      ? await ctx.db.db.select().from(claims).where(inArray(claims.id, claimIds)).limit(12)
      : [];
    ...
    const definitionClaims = acceptedClaims
      .filter((claim) => /definition|define|means|is a|refers to/i.test(claim.claimText))
      .slice(0, 4);
    const formulaClaims = acceptedClaims
      .filter((claim) => /formula|equation|=|\\frac|\\Delta|propto|\^/.test(claim.claimText))
      .slice(0, 6);
    const exampleClaims = acceptedClaims
      .filter((claim) => /example|instance|case|when|for example/i.test(claim.claimText))
      .slice(0, 4);
    const misconceptionClaims = acceptedClaims
      .filter((claim) =>
        /misconception|mistake|confus|not the same|incorrect/i.test(claim.claimText),
      )
      .slice(0, 4);
    ...
    if (conceptWikiPage?.markdown.trim()) {
      ...
      conceptBlocks.push({
        ...
        kind: "summary",
        title: "Overview",
        content: concept.description ?? `${concept.canonicalName} reference.`,
        evidenceRefs: learnerEvidence,
      });
    }
    ...
    return withInteractiveBlocks(
      ...
      base({
        ...
        provenanceRefs: [
          ...chunkEvidence.map((item) => ({
            refType: "chunk" as const,
            refId: item.id,
        ...
        quality: {
          confidence: conceptWikiPage?.qualityScore ?? concept.confidence ?? null,
          sourceBacked:
            chunkEvidence.length > 0 || (conceptWikiPage?.sourceChunkIds ?? []).length > 0,
          needsReview:
            acceptedClaims.length === 0 || claimRows.some((claim) => claim.status === "candidate"),
        },
  ...
  if (openTarget.kind === "wiki_page" && openTarget.entity) {
    ...
    const chunkRefs = (wikiPage.sourceChunkIds ?? []).map((id) => ({
      refType: "chunk" as const,
      refId: id,
    }));
    const claimRefs = (wikiPage.sourceClaimIds ?? []).map((id) => ({
  ...
  if (curriculum) {
    ...
    const curriculumSummary =
      typeof curriculum.scopeJson?.summary === "string"
    ...
    const curriculumPageReadiness = await loadWikiPageReadinessByKey(
      ctx,
      notebookId,
    ...
    return toLearnerFacingReferenceSurface(
      base({
        ...
        blocks: [
          {
            ...
            kind: "markdown",
            title: "Overview",
            content: regeneratedMarkdown ?? markdownPage(curriculum.title, curriculumSummary),
            evidenceRefs: [],
          },
          {
            id: "modules",
        ...
        provenanceRefs: curriculum.sourceIds.map((id) => ({
          refType: "source",
          refId: id,
          role: "derived_from",
        })),
        ...
        quality: {
          confidence: curriculum.confidence ?? null,
          sourceBacked: curriculum.sourceIds.length > 0,
  ...
  if (module) {
    ...
    const orderedObjectives = [...objectiveRows].sort(
      (a, b) => (objectiveOrder.get(a.id) ?? 0) - (objectiveOrder.get(b.id) ?? 0),
    );
    const sourceRefs = Array.isArray(module.sourceRefsJson)
      ...
      : [];
    const modulePageReadiness = await loadWikiPageReadinessByKey(
      ctx,
      notebookId,
      `module:${module.id}`,
    );
        ...
        blocks: [
          {
            ...
            content:
              regeneratedMarkdown ??
              markdownPage(module.title, moduleSummary ?? `${module.title} module reference.`),
            evidenceRefs: [],
          },
          {
            id: "objectives",
            ...
            content: orderedObjectives.map((objective) => ({
              ...
              status:
                objective.id === objectiveList?.currentObjectiveId ? "current" : objective.status,
            })),
            evidenceRefs: [],
          },
        ],
  ...
  if (objectiveList) {
    ...
    const sourceRefs = orderedObjectives.flatMap((objective) =>
      parseSourceRefs(objective.sourceRefsJson),
    );
    const regeneratedMarkdown = jsonString(
    ...
    return toLearnerFacingReferenceSurface(
      base({
        ...
        blocks: [
          {
            ...
            content:
              regeneratedMarkdown ??
              markdownPage(objectiveList.title, "Ordered objectives for this module."),
            evidenceRefs: [],
          },
          {
            id: "objectives",
        ...
        quality: {
          confidence: null,
          sourceBacked: sourceRefs.length > 0,
          needsReview: isWeakPlanningLabel(objectiveList.title),
        },
  ...
  if (objective) {
    ...
    const conceptRefs = conceptIds.map((id) =>
      semanticRef("concept", id, conceptTitleById.get(id)),
    );
    ...
    const linkedArtifacts = linkedArtifactRows
      ...
      .filter(
        (artifact) =>
          Array.isArray(artifact.sourceNodeRefsJson) &&
          artifact.sourceNodeRefsJson.some((ref) => {
      ...
      base({
        ...
        blocks: [
            ...
            : []),
          {
            ...
            content: objective.successCriteriaJson ?? {},
            evidenceRefs: [],
          },
          {
            id: "concepts",
        ...
        quality: {
          confidence: objective.readinessScore ?? null,
          sourceBacked: sourceRefs.length > 0,
  ...
  const [tutorSession] = await ctx.db.db
    .select()
    .from(tutorSessions)
    .where(and(eq(tutorSessions.id, nodeId), eq(tutorSessions.notebookId, notebookId)))
    .limit(1);
  if (tutorSession) {
    ...
    const sourceRefs = sessionRefs.filter(
    ...
    return toLearnerFacingReferenceSurface(
      base({
        ...
        summary:
          runtimeGoal ??
          (orderedTurns.length > 0 ? `${orderedTurns.length} recorded turns.` : null),
        ...
        quality: {
          confidence: null,
          sourceBacked: sourceRefs.length > 0,
          needsReview: tutorSession.status !== "completed" && orderedTurns.length === 0,
        },
      }),
    );
  }
  ...
  if (sessionPlan) {
    ...
    const objectiveRefs = (sessionPlan.plannedObjectiveIds ?? []).map((id) =>
      semanticRef("objective", id),
    );
    const sessionObjectiveRows = sessionPlan.plannedObjectiveIds.length
      ...
      : [];
    const objectiveOrder = new Map(
      sessionPlan.plannedObjectiveIds.map((id, index) => [id, index] as const),
    );
    const sessionObjectives = [...sessionObjectiveRows].sort(
      (a, b) => (objectiveOrder.get(a.id) ?? 0) - (objectiveOrder.get(b.id) ?? 0),
    );
    return toLearnerFacingReferenceSurface(
      base({
        ...
        blocks: [
          regeneratedMarkdown
            ...
            : {
                ...
                kind: "summary",
                title: "Session goal",
                content: sessionPlan.sessionGoal ?? `${sessionPlan.title} session reference.`,
                evidenceRefs: [],
              },
          {
            ...
            content: sessionObjectives.map((objective) => ({
              title: objective.title,
              status: objective.status,
          ...
          },
          {
            ...
            kind: "metadata",
            title: "Opener",
            content: sessionPlan.openerJson ?? {},
            evidenceRefs: [],
          },
          {
            id: "exit",
            kind: "metadata",
            title: "Exit criteria",
            content: sessionPlan.exitCriteriaJson ?? {},
            evidenceRefs: [],
          },
        ],
        scopeRefs: [
        ...
        quality: {
          confidence: null,
          sourceBacked: false,
          needsReview: isWeakPlanningLabel(sessionPlan.title),
        },
      ...
      base({
        ...
        primaryActions:
          artifact.artifactType === "quiz"
            ? ["ask_tutor", "quiz", "regenerate", "open_provenance"]
            : ["ask_tutor", "review", "regenerate", "open_provenance"],
        quality: {
          confidence: view.confidence,
          sourceBacked: view.quality.sourceBacked,
          needsReview: view.quality.needsReview,
    ...
    return toLearnerFacingReferenceSurface(
      base({
        ...
        quality: {
          confidence: null,
          sourceBacked: true,
          needsReview: source.status !== "tutoring_ready",
        },
      }),
    );
  }
      ...
      blocks: [
        {
          ...
          title: "Reference",
          content:
            "Open the tutor chat for teaching, or enable Dev mode to inspect raw graph details.",
          evidenceRefs: [],
        },
      ],
...
export function readablePlanningSummary(
  summary: string | null | undefined,
  title: string,
): string | null {
  const trimmed = summary?.trim();
  if (!trimmed) return null;
  if (/^bootstrap module generated from\b/i.test(trimmed))
    return `Review the source-grounded scope for ${title}.`;
  return trimmed;
}
...
function sectionToReferenceBlock(
  section: ReturnType<typeof buildLearningArtifactView>["sections"][number],
): ReferenceBlock {
  if (section.kind === "markdown") {
    return {
      ...
      kind: "markdown",
      title: section.title,
      content: String(section.content ?? section.emptyMessage ?? ""),
      evidenceRefs: [],
    };
  }
  ...
  if (section.kind === "empty") {
    return {
      ...
      kind: "summary",
      title: section.title,
      content: section.emptyMessage ?? "No content recorded yet.",
      evidenceRefs: [],
    };
  }
  return {
...
function parseSourceRefs(value: unknown): Array<{ refType: "source" | "chunk"; refId: string }> {
  ...
  return value.flatMap((item) => {
    if (!isJsonRecord(item)) return [];
    const refId =
      typeof item.refId === "string" ? item.refId : typeof item.id === "string" ? item.id : null;
    if (!refId) return [];
