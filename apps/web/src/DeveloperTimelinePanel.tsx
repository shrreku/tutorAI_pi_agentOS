...
export function DeveloperTimelinePanel({
  notebookId,
  onSelectNodeRefs,
}: DeveloperTimelinePanelProps) {
  const { data, isLoading, error } = useQuery({
    queryKey: ["developer-timeline", notebookId],
    queryFn: async (): Promise<DeveloperTimelineResponse> => {
      const response = await fetch(
        `/api/v1/notebooks/${encodeURIComponent(notebookId)}/developer/timeline?limit=250`,
      );
      if (!response.ok) {
  ...
  const visibleItems = showRawEvents
    ? items
    : items.filter((item) => item.eventType !== "tutor.message.delta");
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
        ...
        {data && (
          <div
            style={{ display: "flex", gap: 8, color: "#6b7280", fontSize: 11, flexWrap: "wrap" }}
          >
        ...
        {error instanceof Error && (
          <span style={{ fontSize: 11, color: "#b91c1c" }}>{error.message}</span>
        )}
        <button
          type="button"
      ...
      {!isLoading && items.length === 0 && !error && (
        <div style={{ fontSize: 12, color: "#6b7280", padding: "4px 0" }}>
          No developer activity found yet.
        </div>
      )}
              ...
              >
                ...
                >
                  <span style={{ minWidth: 0 }}>
                    ...
                    >
                      <span style={{ fontSize: 12, fontWeight: 800, color: "#111827" }}>
                        {isCollapsed ? "▸" : "▾"} {summary.agentLabel}
                      </span>
                      {summary.status && <StatusPill status={summary.status} />}
                      {summary.model && (
                        <span style={{ fontSize: 11, color: "#6b7280" }}>{summary.model}</span>
                      )}
                      {summary.duration && (
                        <span style={{ fontSize: 11, color: "#6b7280" }}>{summary.duration}</span>
                      )}
                    </span>
                    <span
                      style={{
                  ...
                  <span style={{ fontSize: 11, color: "#6b7280", whiteSpace: "nowrap" }}>
                    {summary.toolCount} tools · {summary.updateCount} updates · {group.items.length}{" "}
                    events
                  </span>
                </button>
                {!isCollapsed && (
...
function TimelineItem({
  ...
  item: DeveloperTimelineResponse["items"][number];
  onSelectNodeRefs?: (refs: NodeRef[]) => void;
}) {
  const kindKey = (
    item.kind && item.kind in KIND_STYLES ? item.kind : "event"
  ) as keyof typeof KIND_STYLES;
        ...
        {(item.model || item.promptVersion || item.traceId || item.usage) && (
          <div style={{ fontSize: 11, color: "#6b7280", marginTop: 3 }}>
            {[
              ...
              item.promptVersion ? `prompt ${item.promptVersion}` : null,
              item.traceId ? `trace ${item.traceId.slice(0, 8)}` : null,
              item.usage ? formatUsage(item.usage) : null,
            ]
              .filter(Boolean)
              .join(" · ")}
          </div>
...
function groupTimelineItems(items: DeveloperTimelineResponse["items"]) {
  const groups = new Map<
    string,
    { id: string; title: string; items: DeveloperTimelineResponse["items"] }
  >();
  for (const item of items) {
    const key = item.runId
      ...
      : item.sessionId
        ...
        : "activity";
    const title = item.runId
      ? `Run ${item.runId.slice(0, 10)}`
      : item.sessionId
        ? `Session ${item.sessionId.slice(0, 10)}`
        : "Notebook activity";
...
function summarizeTimelineGroup(items: DeveloperTimelineResponse["items"]) {
  const run =
    items.find((item) => item.kind === "agent_run" && item.runId) ??
    items.find((item) => item.runId);
  const tools = items
  ...
  const stateUpdates = items.filter(
    (item) =>
      item.kind === "artifact_change" ||
      item.kind === "wiki_change" ||
      item.kind === "ingestion_job",
  );
  ...
  const agentLabel = run?.title
    ? displayRunTitle(run.title)
    : run?.runId
      ? `Run ${run.runId.slice(0, 10)}`
      : "Notebook activity";
  const visibleLine = [
    tools.length
      ? `Tools: ${tools
