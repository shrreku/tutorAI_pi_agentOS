...
export function buildTutorPromptForReferenceAction(surface: ReferenceSurface, actionId: ReferenceSurfaceActionId): string {
  if (actionId === "quiz") return `Give me source-grounded practice for "${surface.title}".`;
  if (actionId === "review") return `Review "${surface.title}" with me and focus on what I should understand next.`;
  return `Teach me "${surface.title}" using the selected reference.`;
}

export function referenceSurfaceSupportsRegeneration(surface: ReferenceSurface | null | undefined): boolean {
