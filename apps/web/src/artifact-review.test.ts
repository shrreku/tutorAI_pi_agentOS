...
describe("buildArtifactReviewView", () => {
  ...
  it("keeps approval actions consistent for proposed artifacts", () => {
    expect(
      buildArtifactReviewView({
        ...
        view: {
          confidence: null,
          quality: { sourceBacked: false, needsReview: true, issues: ["Needs citations"] },
        },
      }).actions,
    ).toEqual(["approve", "reject", "review", "ask_tutor", "save"]);
  });
  ...
  it("matches Reference Surface action families for artifact types", () => {
    ...
    ).toEqual(["ask_tutor", "quiz", "regenerate", "open_evidence"]);
    expect(
      artifactSurfaceActionIds({
        id: "note_1",
        title: "Note",
    ...
    ).toEqual(["ask_tutor", "review", "regenerate", "open_evidence"]);
  });
});
