  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap font-medium transition-[background,border,color,box-shadow,transform] duration-150 disabled:pointer-events-none disabled:opacity-50 focus-visible:outline-2 focus-visible:outline-offset-2 active:translate-y-px select-none",
  {
    variants: {
      variant: {
        ...
        gold: "bg-gold text-white hover:opacity-90 shadow-soft",
...
export function Meter({
  ...
  value: number;
  tone?: "primary" | "accent" | "gold" | "success" | "warning" | "danger";
  className?: string;
  thickness?: "sm" | "md" | "lg";
}) {
  const fill = {
...
export function Ring({
  ...
  const color =
    tone === "primary" ? "var(--primary)" : tone === "gold" ? "var(--gold)" : "var(--accent)";
  return (
    <div className="relative inline-grid place-items-center" style={{ width: size, height: size }}>
      <div
        className="absolute inset-0 rounded-full"
        style={{
          background: `conic-gradient(${color} ${v}%, color-mix(in oklab, var(--muted-foreground) 20%, transparent) 0)`,
...
export function Skeleton({ className }: { className?: string }) {
  return (
    <div
      ...
      style={{
        backgroundImage:
          "linear-gradient(100deg, transparent 30%, color-mix(in oklab, var(--card) 60%, transparent) 50%, transparent 70%)",
...
export function Input({ className, ...props }: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      className={cn(
        "h-10 w-full rounded-lg border border-input bg-elevated px-3 text-sm text-foreground outline-none transition-shadow placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-accent/30",
        className,
      )}
      {...props}
    />
...
export function Textarea({ className, ...props }: TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      className={cn(
        "w-full resize-none rounded-lg border border-input bg-elevated px-3 py-2 text-sm text-foreground outline-none transition-shadow placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-accent/30",
        className,
      )}
      {...props}
    />
