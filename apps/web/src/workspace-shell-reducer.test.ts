...
describe("workspaceShellReducer", () => {
  it("selects a node by opening the Reference Surface viewer", () => {
    expect(
      workspaceShellReducer(initialWorkspaceShellState, { type: "selectNode", nodeId: "node_1" }),
    ).toMatchObject({
  ...
  it("normalizes impossible viewer and evidence states when selection disappears", () => {
    const selected = workspaceShellReducer(initialWorkspaceShellState, {
      type: "selectNode",
      nodeId: "node_1",
    });
    const withEvidence = workspaceShellReducer(selected, { type: "toggleEvidence" });
    expect(
      workspaceShellReducer(withEvidence, {
  ...
  it("turns learner mode back into a non-diagnostic surface when Dev Mode is disabled", () => {
    ...
    const withEvidence = workspaceShellReducer(selected, { type: "toggleEvidence" });
    expect(
      workspaceShellReducer(withEvidence, { type: "setDeveloperMode", enabled: false })
        .showEvidence,
    ).toBe(false);
  });
  ...
  it("keeps curriculum navigation in workspace mode", () => {
    ...
    expect(
      workspaceShellReducer(selected, { type: "setViewMode", viewMode: "curriculum" }),
    ).toMatchObject({
      viewMode: "curriculum",
      rightPanelMode: "workspace",
      showEvidence: false,
  ...
  it("tracks graph filters and layout resets in shell state", () => {
    const filtered = workspaceShellReducer(initialWorkspaceShellState, {
      type: "toggleTypeFilter",
      filter: "concept",
    });
    ...
    expect(workspaceShellReducer(cleared, { type: "incrementLayoutVersion" }).layoutVersion).toBe(
      1,
    );
  });
});
