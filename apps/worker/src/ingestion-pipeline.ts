...
export async function processIngestionPipelineJob(input: {
  ...
  const { notebookId, sourceId, sourceVersionId } = job.data;
  ...
  try {
    await startActiveObservation(
      ...
      async (jobObservation: any) => {
        ...
        const sourceReadiness = buildSourceReadinessDuringIngestion({
          retrievalReady: retrievalChunks.length > 0 && lexicalReady,
          enrichmentOk,
          ...(enrichmentReason ? { enrichmentReason } : {}),
          tutoringReady: tutoringGate,
          projectionConfigured: Boolean(env.NEO4J_URI && env.NEO4J_PASSWORD),
