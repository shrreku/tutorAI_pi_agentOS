...
function activeTheme() {
  ...
  if (t && THEME_META[t]) { try { localStorage.setItem('tb-theme', t); } catch (e) {} return t; }
...
Object.assign(ICONS, {
  ...
  command: svg('<path d="M18 3a3 3 0 0 0-3 3v12a3 3 0 1 0 3-3H6a3 3 0 1 0 3 3V6a3 3 0 1 0-3 3h12a3 3 0 1 0-3-3"/>'),
...
function themeChrome(part, ...args) {
  ...
  const core = { topbar: coreTopbar, statusbar: coreStatusbar, tutor: coreTutor,
    evidencePanel: coreEvidencePanel, drawer: coreDrawer, dashboard: coreDashboard };
  return core[part] ? core[part](...args) : '';
}

function renderChrome() {
  // slot-based includes
  document.querySelectorAll('[data-include="topbar"]').forEach(el=>{ el.classList.add('topbar'); el.innerHTML = themeChrome('topbar'); });
  document.querySelectorAll('[data-include="statusbar"]').forEach(el=>{ el.classList.add('statusbar'); el.innerHTML = themeChrome('statusbar'); });
  document.querySelectorAll('[data-include="dashboard"]').forEach(el=>{ el.innerHTML = themeChrome('dashboard'); });
  document.querySelectorAll('[data-include="tutor"]').forEach(el=>{ el.innerHTML = themeChrome('tutor', el.dataset.context || 'SN2 mechanism'); });
  document.querySelectorAll('[data-include="drawer"]').forEach(el=>{ el.classList.add('drawer'); el.setAttribute('data-drawer',''); el.innerHTML = themeChrome('drawer'); });
  document.querySelectorAll('[data-include="evidence-panel"]').forEach(el=>{ el.innerHTML = themeChrome('evidencePanel'); });
  ...
  document.querySelectorAll('.workspace[data-evidence]').forEach(ws=>{
    ...
    const wrap = document.createElement('div'); wrap.innerHTML = themeChrome('evidencePanel');
    Array.from(wrap.children).forEach(c=>ws.appendChild(c));
  });

  // left chat drawer for reference pages
  document.querySelectorAll('[data-chat-drawer], [data-chat-drawer-scrim]').forEach(n=>n.remove());
  if (document.querySelector('.ref-page')) {
    const ctx = document.body.dataset.chatContext || 'SN2 mechanism';
    const wrap = document.createElement('div'); wrap.innerHTML = coreChatDrawer(ctx);
    Array.from(wrap.children).forEach(c=>document.body.appendChild(c));
    const stray = document.querySelector('[data-chat-drawer] .chat-fab'); if (stray) stray.remove();
  }

  // floating nav
  document.querySelectorAll('[data-floatnav]').forEach(n=>n.remove());
  const navWrap = document.createElement('div'); navWrap.innerHTML = floatNav();
  document.body.appendChild(navWrap.firstElementChild);

  // command palette (once)
  if (!document.querySelector('[data-palette]')) {
    const pal = document.createElement('div');
    pal.className = 'palette-scrim hidden'; pal.setAttribute('data-palette','');
