...
export function composeTeachingArc(input: ComposeTeachingArcInput): TeachingArc {
  ...
  const concepts = input.targetConceptNames?.length
    ? input.targetConceptNames.join(", ")
    : input.objectiveTitle;
  const coverageItemIds = (input.mustCoverItems ?? [])
    .map((item) => item.id)
    .filter((id): id is string => Boolean(id));
  const sourceRefs = (input.mustCoverItems ?? [])
    .flatMap((item) => item.sourceRefs ?? [])
    .slice(0, 6);
  const formulaItems = (input.mustCoverItems ?? []).filter(
    (item) => item.itemFamily === "formula" || item.itemFamily === "notation",
  );
  const misconceptionItems = (input.mustCoverItems ?? []).filter(
    (item) => item.itemFamily === "misconception" || item.itemFamily === "distinction",
  );
  const mechanismItems = (input.mustCoverItems ?? []).filter(
    (item) => item.itemFamily === "procedure" || item.itemFamily === "application",
  );
  const analogyItems = (input.mustCoverItems ?? []).filter(
    (item) =>
      item.itemFamily === "distinction" ||
      item.itemFamily === "application" ||
      item.itemFamily === "misconception",
  );
  ...
  const blocks: TeachingArcBlock[] = [
    block(
      ...
      "hook",
      "Why this matters",
      `Connect ${input.objectiveTitle} to the learner's goal before definitions.`,
      sourceRefs,
    ),
    block(
      input.objectiveId,
      "prior_knowledge_probe",
      "Quick diagnostic",
      `Ask one short question to check prerequisite intuition for ${concepts}.`,
      sourceRefs,
    ),
    block(
      input.objectiveId,
      "intuition",
      "Core intuition",
      `Explain the idea in plain language at a ${pace} pace.`,
      sourceRefs,
    ),
    block(
      input.objectiveId,
      "formal_definition",
      "Formal shape",
      `State the rigorous definition or rule for ${input.objectiveTitle}; keep depth ${depth}.`,
      sourceRefs,
    ),
  ];
  ...
  blocks.push(
    block(
      ...
      "everyday_example",
      "Concrete example",
      `Use a ${exampleStyle} example to make ${input.objectiveTitle} tangible.`,
      sourceRefs,
    ),
    block(
      input.objectiveId,
      "industrial_example",
      "Applied example",
      `Show one realistic application or workflow involving ${concepts}.`,
      sourceRefs,
    ),
  );
  ...
  if (misconceptionItems.length) {
    ...
    blocks.push(
      block(
        ...
        "contrast_case",
        "Contrast case",
        `Contrast ${input.objectiveTitle} with a nearby idea to prevent overgeneralization.`,
        sourceRefs,
      ),
    );
  }
  ...
  blocks.push(
    block(
      ...
      "checkpoint",
      "Checkpoint",
      `Ask a compact check-for-understanding question before moving on.`,
      sourceRefs,
    ),
    block(
      input.objectiveId,
      "transfer_prompt",
      "Transfer",
      `Ask the learner to apply ${input.objectiveTitle} in a new situation.`,
      sourceRefs,
    ),
    block(
      input.objectiveId,
      "summary",
      "Summary",
      `Summarize what was introduced, what was checked, and what remains weak.`,
      sourceRefs,
    ),
  );
...
export function adaptTeachingArcForRuntime(
  arc: TeachingArc,
  state: TeachingArcRuntimeState = {},
): AdaptedTeachingArc {
  ...
  if (
    (state.recentMistakeConceptIds?.length ?? 0) > 0 ||
    (state.weakCoverageItemIds?.length ?? 0) > 0
  ) {
    priorityMove(ordered, [
      "misconception_warning",
      "contrast_case",
      "everyday_example",
      "checkpoint",
  ...
  return {
    ...arc,
    blocks: ordered,
    activeBlock: nextBlocks[0] ?? null,
    nextBlocks: nextBlocks.slice(0, 3),
    adaptationReason,
  };
}
