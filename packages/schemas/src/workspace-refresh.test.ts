...
import {
  ...
  workspaceRefreshPolicyForEvent,
} from "./workspace-refresh.js";

describe("workspace refresh contract", () => {
  it("authors refresh hints for reference regeneration events", () => {
    expect(
      workspaceRefreshPolicyForEvent("reference.regenerated", {
        targets: [],
        nodeIds: ["artifact_1"],
        artifactIds: [],
        sourceIds: [],
      }).targets,
    ).toEqual(expect.arrayContaining(["graph", "referenceSurfaces", "curriculum"]));
  });

  it("authors quiz attempt refresh hints scoped to the artifact", () => {
    expect(
      workspaceRefreshPolicyForEvent("quiz.attempt.recorded", {
        targets: [],
        nodeIds: [],
        artifactIds: ["artifact_1"],
        sourceIds: [],
      }).targets,
    ).toEqual(expect.arrayContaining(["graph", "studyState", "quizAttempts", "artifacts"]));
  });
  ...
  it("invalidates source files when source lifecycle events fire", () => {
    expect(resolveWorkspaceRefreshPolicy("source.uploaded").targets).toEqual([
      "sources",
      "sourceFiles",
    ]);
  ...
  it("merges server hints with event-derived refresh targets", () => {
    expect(
      resolveWorkspaceRefreshPolicy("artifact.updated", {
