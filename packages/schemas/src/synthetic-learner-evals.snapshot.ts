...
export const persistedEvalNotebookStateSchema = z.object({
  notebookId: idSchema,
  masteryEvidence: z.array(z.object({
    id: idSchema,
  ...
  notebookEvents: z.array(z.object({
    id: idSchema,
    eventType: z.string().min(1),
    sessionId: idSchema.optional(),
    timestamp: z.string().datetime().optional(),
  ...
  explicitLearnerGoals: z.array(z.object({
    refType: z.literal("notebook"),
    refId: idSchema,
    summary: z.string().min(1),
  })).default([]),
  learningStates: z.array(z.object({
    conceptId: idSchema,
  ...
  personalizationRecommendations: z.array(z.object({ id: idSchema, trait: z.string().min(1) })).default([]),
});
...
export const evalEvidenceSnapshotSchema = z.object({
  ...
  readinessStates: z.array(z.object({
    sourceId: idSchema,
  ...
  personalizationRecommendations: persistedEvalNotebookStateSchema.shape.personalizationRecommendations.optional(),
  traitRecommendationOnlySnapshot: z.object({
  ...
  sessionEventRecords: z.array(z.object({
...
export type EvalEvidenceSnapshotCategoryRecord = z.infer<typeof evalEvidenceSnapshotCategoryRecordSchema>;
...
const PERSISTENCE_ASSERTION_REQUIRED_CATEGORIES: Record<string, EvalEvidenceSnapshotCategory[]> = {
  ...
  persistence_trait_estimates: ["session_lifecycle", "learner_trait_signals", "learner_trait_estimates"],
...
export function requiredSnapshotCategoriesForAssertionRefs(
  ...
  return [...new Set(assertionRefs.flatMap((ref) => PERSISTENCE_ASSERTION_REQUIRED_CATEGORIES[ref.refId] ?? []))];
}
...
export function captureEvalEvidenceSnapshotFromPersistedState(input: {
  ...
  return buildEvalEvidenceSnapshot({
    ...
    artifacts: state.artifacts.map((entry) => ({ ref: { refType: "artifact", refId: entry.id }, status: entry.status })),
    quizAttempts: state.quizAttempts.map((entry) => ({ ref: { refType: "turn", refId: entry.id } })),
    ...
    learnerTraitSignals: state.learnerTraitSignals.map((entry) => ({ ref: { refType: "trait_signal", refId: entry.id } })),
    learnerTraitEstimates: state.learnerTraitEstimates.map((entry) => ({ ref: { refType: "trait_estimate", refId: entry.id } })),
...
export function buildEvalEvidenceSnapshot(input: BuildEvalEvidenceSnapshotInput): EvalEvidenceSnapshot {
  ...
  const categories: EvalEvidenceSnapshotCategoryRecord[] = [
    categoryRecord("mastery_evidence", input.masteryEvidence?.map((entry) => entry.ref) ?? [], requiredCategories.has("mastery_evidence")),
    categoryRecord("artifacts", input.artifacts?.map((entry) => entry.ref) ?? [], requiredCategories.has("artifacts")),
    categoryRecord("quiz_attempts", input.quizAttempts?.map((entry) => entry.ref) ?? [], requiredCategories.has("quiz_attempts"), "no_quiz_attempt_records"),
    categoryRecord("session_lifecycle", input.sessionEvents?.flatMap((entry) => (entry.ref ? [entry.ref] : [])) ?? [], requiredCategories.has("session_lifecycle"), "no_session_lifecycle_records"),
    categoryRecord("tutor_turns", input.tutorTurns?.map((entry) => entry.ref) ?? [], requiredCategories.has("tutor_turns"), "no_tutor_turn_records"),
    categoryRecord("tool_calls", input.toolCalls?.map((entry) => entry.ref) ?? [], requiredCategories.has("tool_calls"), "no_tool_call_records"),
    categoryRecord("learner_trait_signals", input.learnerTraitSignals?.map((entry) => entry.ref) ?? [], requiredCategories.has("learner_trait_signals"), "no_trait_signal_records"),
    categoryRecord("learner_trait_estimates", input.learnerTraitEstimates?.map((entry) => entry.ref) ?? [], requiredCategories.has("learner_trait_estimates"), "no_trait_estimate_records"),
    categoryRecord("workspace_reference_surfaces", input.workspaceReferenceSurfaces?.map((entry) => entry.ref) ?? [], requiredCategories.has("workspace_reference_surfaces"), "no_workspace_reference_surface_records"),
    categoryRecord("notebook_events", input.notebookEvents?.flatMap((entry) => (entry.ref ? [entry.ref] : [])) ?? [], requiredCategories.has("notebook_events"), "no_notebook_event_records"),
  ];
  ...
  return evalEvidenceSnapshotSchema.parse({
    ...
    ...(input.artifacts ? { artifactsMeta: input.artifacts.map((entry) => ({ id: entry.ref.refId, status: entry.status })) } : {}),
    ...
    ...(input.personalizationRecommendations ? { personalizationRecommendations: input.personalizationRecommendations } : {}),
    ...(input.traitRecommendationOnlySnapshot ? { traitRecommendationOnlySnapshot: input.traitRecommendationOnlySnapshot } : {}),
...
export function evalEvidenceSnapshotToPersistenceEvidence(
  ...
  if (
    ...
    mergedSessionEvents.push(
      ...traitSignalCategory.refs.map((ref) => ({ ref, eventType: "learner_trait.signal.recorded" })),
    );
  }
    ...
    ...(artifactCategory?.refs.length
      ? {
          artifacts: artifactCategory.refs.map((ref) => ({
            ref,
            status: snapshot.artifactsMeta?.find((entry) => entry.id === ref.refId)?.status ?? "ready",
          })),
        }
      : {}),
    ...(mergedSessionEvents.length ? { sessionEvents: mergedSessionEvents } : {}),
    ...(snapshot.traitRecommendationOnlySnapshot ? { traitRecommendationOnlySnapshot: snapshot.traitRecommendationOnlySnapshot } : {}),
  };
}
...
export function extractForbiddenProductStateFromSnapshot(snapshot: EvalEvidenceSnapshot): SyntheticLearnerForbiddenProductStateSnapshot {
  const byCategory = new Map(snapshot.categories.map((entry) => [entry.category, entry]));
  const refs = (category: EvalEvidenceSnapshotCategory) => byCategory.get(category)?.refs ?? [];
  return {
    masteryEvidenceRefs: refs("mastery_evidence"),
    learningStateRefs: (snapshot.learningStates ?? []).map((entry) => ({ refType: "concept", refId: entry.conceptId })),
    weakConceptRefs: (snapshot.weakConceptIds ?? []).map((conceptId) => ({ refType: "weak_concept", refId: conceptId })),
    objectiveRefs: (snapshot.objectives ?? []).map((entry) => ({ refType: "objective", refId: entry.id })),
    curriculumRefs: [
      ...(snapshot.curricula ?? []).map((entry): NodeRef => ({ refType: "curriculum", refId: entry.id })),
      ...(snapshot.curriculumModules ?? []).map((entry): NodeRef => ({ refType: "curriculum_module", refId: entry.id })),
    ],
    studyPlanRefs: (snapshot.studyPlans ?? []).map((entry) => ({ refType: "study_plan", refId: entry.id })),
    artifactRefs: refs("artifacts"),
    sourceGroundingRefs: refs("workspace_reference_surfaces").filter((ref) => ref.refType === "source" || ref.refType === "chunk"),
    ...
    personalizationRecommendationRefs: (snapshot.personalizationRecommendations ?? []).map((entry) => ({
...
export function missingRequiredSnapshotCategories(snapshot: EvalEvidenceSnapshot): EvalEvidenceSnapshotCategory[] {
...
export function mergeEvalEvidenceSnapshots(input: {
  ...
  const explicitLearnerGoals = input.supplemental.explicitLearnerGoals ?? input.persisted.explicitLearnerGoals;
  ...
  const curriculumModules = input.supplemental.curriculumModules ?? input.persisted.curriculumModules;
  const personalizationRecommendations =
    input.supplemental.personalizationRecommendations ?? input.persisted.personalizationRecommendations;
  const traitRecommendationOnlySnapshot =
    input.supplemental.traitRecommendationOnlySnapshot ?? input.persisted.traitRecommendationOnlySnapshot;
  const persistedSessionEvents = input.persisted.sessionEventRecords?.flatMap((entry) =>
    entry.ref ? [{ ref: entry.ref, eventType: entry.eventType, ...(entry.timestamp ? { timestamp: entry.timestamp } : {}) }] : [],
  );
  const merged = buildEvalEvidenceSnapshot({
    ...
    requiredCategories: input.persisted.categories.filter((entry) => entry.required).map((entry) => entry.category),
    ...(input.supplemental.masteryEvidence ? { masteryEvidence: input.supplemental.masteryEvidence } : {}),
    ...
    ...(input.supplemental.learnerTraitSignals ? { learnerTraitSignals: input.supplemental.learnerTraitSignals } : {}),
    ...(input.supplemental.learnerTraitEstimates ? { learnerTraitEstimates: input.supplemental.learnerTraitEstimates } : {}),
    ...(input.supplemental.workspaceReferenceSurfaces ? { workspaceReferenceSurfaces: input.supplemental.workspaceReferenceSurfaces } : {}),
    ...(input.supplemental.notebookEvents ? { notebookEvents: input.supplemental.notebookEvents } : {}),
