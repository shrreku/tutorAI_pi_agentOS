...
const llmHandleSchema = z.object({
  id: z.string().min(1),
  ref: nodeRefSchema,
...
const conceptHandleSchema = llmHandleSchema.extend({
  name: z.string().min(1).optional(),
});

const objectiveHandleSchema = llmHandleSchema.extend({
  title: z.string().min(1),
  status: z.string().min(1),
  orderIndex: z.number().int().optional(),
  targetConcepts: z.array(conceptHandleSchema).default([]),
  prerequisiteConcepts: z.array(conceptHandleSchema).default([]),
});

const boundarySignalSchema = z.object({
  type: z.enum([
  ...
  strength: z.enum(["weak", "likely", "confirmed"]),
...
const wikiSearchOutputSchema = wikiSearchResponsePayloadSchema;
...
const curriculumGetOutputSchema = z.object({
  curriculum: z
    .object({
      ...
      objectives: z.array(objectiveHandleSchema).default([]),
    })
    .nullable(),
});
...
const studyPlanGetCurrentOutputSchema = z.object({
  studyPlan: z
    .object({
      ...
      currentObjective: objectiveHandleSchema.nullable().optional(),
      upcomingObjectives: z.array(objectiveHandleSchema).default([]),
      completedObjectives: z.array(objectiveHandleSchema).default([]),
      weakConcepts: z.array(conceptHandleSchema).default([]),
    })
    .nullable(),
  studentProfile: z
    .object({
  ...
  objectiveList: z
    .object({
      ...
      currentObjective: objectiveHandleSchema.nullable().optional(),
      objectivesOrdered: z.array(objectiveHandleSchema).default([]),
  ...
  sessionPlan: z
    .object({
      ...
      plannedObjectives: z.array(objectiveHandleSchema).default([]),
...
export function createNoopRuntimeReadToolProvider(): RuntimeReadToolProvider {
  ...
  return {
    ...
    async wikiSearch() {
      return { results: [], warnings: [], boundarySignals: [] };
    },
    async wikiGetPage() {
      return { page: null };
    },
