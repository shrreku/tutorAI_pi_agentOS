...
describe("planMockTutorSessionSteps", () => {
  it("routes quiz requests to artifact.create_quiz", () => {
    const steps = planMockTutorSessionSteps("Can you make me a quiz I can study from?", baseContext);
    expect(steps.some((step) => step.type === "tool_call" && step.toolName === "artifact.create_quiz")).toBe(true);
  });
  ...
  it("routes remediation corrections to learning.evaluate_response", () => {
    ...
    expect(steps.some((step) => step.type === "tool_call" && step.toolName === "learning.evaluate_response")).toBe(true);
  });

  it("routes slope misconceptions to a mastery-check question", () => {
    const steps = planMockTutorSessionSteps("I think the rule is about slope, but I may be mixing things up.", baseContext);
  ...
  it("keeps Fourier's law fallback tutoring on the requested source topic", () => {
    const steps = planMockTutorSessionSteps(
      ...
      { ...baseContext, notebookTitle: "Heat transfer", selectedNodeRefs: [{ refType: "source", refId: "src_1" }] },
    );
    const text = steps.filter((step) => step.type === "text").map((step) => step.content).join(" ");
    ...
    expect(steps.some((step) => step.type === "tool_call" && step.toolName === "wiki.search")).toBe(true);
  });
  ...
  it("uses grounded context fallback instead of generic idle greeting", () => {
    ...
    expect(steps.some((step) => step.type === "tool_call" && step.toolName === "notebook.get_context")).toBe(true);
  });

  it("routes revision follow-ups to artifact creation", () => {
    const steps = planMockTutorSessionSteps("Please keep it tied to the source and make it useful for revision.", baseContext);
    expect(steps.some((step) => step.type === "tool_call" && step.toolName === "artifact.create_quiz")).toBe(true);
  });
});
