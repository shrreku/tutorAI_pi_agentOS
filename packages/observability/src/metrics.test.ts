...
describe("MetricRegistry", () => {
  it("renders counters, gauges, and histograms in Prometheus text format", () => {
    ...
    registry.incrementCounter("studyagent_test_total", 2, { route: "/health", method: "GET" }, "Test counter.");
    registry.setGauge("studyagent_test_active", 3, { worker: "api" }, "Test gauge.");
    registry.observeHistogram("studyagent_test_duration_seconds", 0.02, { phase: "run" }, "Test histogram.", [0.01, 0.05]);
  ...
  it("records standard low-cardinality StudyAgent metrics", () => {
    ...
    recordHttpRequestMetric({ method: "post", route: "/api/v1/notebooks/:notebookId/tutor/chat", statusCode: 201, durationMs: 42, registry });
    ...
    expect(rendered).toContain('studyagent_http_requests_total{method="POST",route="/api/v1/notebooks/:notebookId/tutor/chat",status_class="2xx"} 1');
